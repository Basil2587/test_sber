...
def parse_page_2(page_2):
    ...
    res = res.append(pd.DataFrame([[date_reg, date_close]],
...
def parse_page_right(page_right):
    ...
    res = res.append(pd.DataFrame([[inform_of_the_right]],
        columns=['inform_of_the_right']), ignore_index=True)
    return res
...
if __name__ == '__main__':
    ...
    for i in main():
        ...
        page_arch = find_archive(page_archive)
        page_right = find_right(page_get_right)
        result = pd.DataFrame()
        for item in tables:
            res = parse_table(item)
        data_date = parse_page_2(page_2)
        archive = parse_page_arch(page_arch)
        the_right = parse_page_right(page_right)
